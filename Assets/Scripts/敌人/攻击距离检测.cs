...
public class 攻击距离检测 : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // 确保字典初始化
        _controller.攻击距离检测 ??= new Dictionary<GameObject, bool>();
    }
    ...
    private void OnTargetEnter(GameObject target)
    {
        ...
        _controller.攻击距离检测[target] = true;
        _controller.追踪或攻击玩家();
    }
    ...
    private void OnTargetExit(GameObject target)
    {
        ...
        if (_controller.攻击距离检测.ContainsKey(target))
        {
            _controller.攻击距离检测.Remove(target);
        }
        _controller.追踪或攻击玩家();
    }
    ...
    private void OnDisable()
    {
        ...
        {
            if (target != null && _controller.攻击距离检测.ContainsKey(target))
            {
                _controller.攻击距离检测.Remove(target);
            }
        }
